...
public class BootCompletedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        Log.d("ZMAX", "Starting BootCompletedReceiver");
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        if("1".equals(prefs.getString("keys_order", "0"))) {
            try {
                Log.d("ZMAX", "Trying to change touch key layout");
        ...
        if(fast_charge) {
            try {
                Log.d("ZMAX", "Trying to force fast charge");
            ...
            if(!"0".equals(fast_charge_value)) try {
                Log.d("ZMAX", "Trying to set fast charge value");
        ...
        try {
            Log.d("ZMAX", "Trying to change palm2sleep value");
        ...
        try {
            Log.d("ZMAX", "Trying to change cpu_boost_freq value");
        ...
        try {
            Log.d("ZMAX", "Trying to change cpu_boost_cores value");
        ...
        context.startService(new Intent(context, ZMAXDozeService.class));
    }
}
